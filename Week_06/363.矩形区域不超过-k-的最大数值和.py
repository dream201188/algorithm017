...
首先，比如说[3, -1, 2, 4, -3, 2]，对于这个数组里我们假如取index为(2, 4)之间的子序列和（记为sum[2, 4]），即取[2, 4, -3]的和。而[2, 4, -3]的
其实会等于[3, -1, 2, 4, -3]的和减去[3, -1]的和。因此我们可以有这个结论：
...
这样的话我们可以把初始化一个cum，同时依次把数组的值加进去。每加一个，就把这个值丢到一个箱子里。并且每次把这个cum - k，并且在箱子里找到比cum - k大的值
（这些值可能很多个，但我们只需要最小的那个）。为了查找方便，我们就把这个箱子维护成排序的箱子。
